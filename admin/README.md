# Topup Online Admin - 后台管理系统

这是一个基于 Vue 3 + Naive UI + Tailwind CSS 的后台管理系统，用于管理充值订单、用户、卡密等业务。

## 技术栈

- **Vue 3** - 渐进式JavaScript框架
- **Vite** - 下一代前端构建工具
- **Naive UI** - Vue 3 组件库
- **Tailwind CSS** - 实用优先的CSS框架
- **TypeScript** - JavaScript的超集
- **Pinia** - Vue状态管理库
- **Vue Router** - Vue官方路由

## 项目结构

```
admin/
├── src/
│   ├── components/      # 通用组件
│   │   └── AppContent.vue
│   ├── layouts/         # 布局组件
│   │   └── AdminLayout.vue  # 后台管理布局
│   ├── views/           # 页面组件
│   │   ├── Login.vue        # 登录页面
│   │   ├── Dashboard.vue    # 控制台
│   │   ├── Users.vue        # 用户管理
│   │   ├── Orders.vue       # 订单管理
│   │   └── Settings.vue     # 系统设置
│   ├── router/          # 路由配置
│   │   └── index.ts
│   ├── styles/          # 全局样式
│   │   └── index.css
│   ├── App.vue          # 根组件
│   └── main.ts          # 入口文件
├── public/              # 公共资源
├── index.html           # HTML模板
├── vite.config.ts       # Vite配置
├── tailwind.config.js   # Tailwind配置
├── postcss.config.js    # PostCSS配置
├── tsconfig.json        # TypeScript配置
└── package.json         # 项目配置
```

## 快速开始

### 安装依赖

```bash
npm install
```

### 启动开发服务器

```bash
npm run dev
```

项目将在 http://localhost:3001 运行

### 构建生产版本

```bash
npm run build
```

### 预览生产版本

```bash
npm run preview
```

## 功能模块

### 登录页面 (`/login`)
- ✅ 用户名密码登录
- ✅ 记住我功能
- ✅ 表单验证
- ✅ 演示账号：`admin` / `admin`

### 控制台 (`/admin/dashboard`)
- ✅ 数据统计卡片（订单、收入、用户、卡密）
- ✅ 趋势对比
- ✅ 图表展示区域（预留）
- ✅ 最近订单列表

### 用户管理 (`/admin/users`)
- ✅ 用户列表展示
- ✅ 搜索和筛选
- ✅ 添加/编辑/删除用户
- ✅ 用户状态管理
- ✅ 角色分配

### 订单管理 (`/admin/orders`)
- ✅ 订单列表展示
- ✅ 订单搜索和筛选
- ✅ 日期范围筛选
- ✅ 订单详情查看
- ✅ 订单状态管理
- ✅ 订单取消功能
- ✅ 数据导出（预留）

### 系统设置 (`/admin/settings`)
- ✅ 基础设置（网站配置）
- ✅ 支付设置（支付方式、金额限制、手续费）
- ✅ 通知设置（邮件、短信、SMTP配置）
- ✅ 多标签页设计

### 布局特性
- ✅ 左侧菜单栏（可折叠）
- ✅ 顶部导航栏
- ✅ 面包屑导航
- ✅ 主题切换（亮色/暗色）
- ✅ 用户下拉菜单
- ✅ 返回前台按钮
- ✅ 路由守卫（登录验证）

## 路由说明

- `/` - 重定向到登录页
- `/login` - 登录页面
- `/admin` - 后台管理（需要登录）
  - `/admin/dashboard` - 控制台
  - `/admin/users` - 用户列表
  - `/admin/roles` - 角色管理（待开发）
  - `/admin/orders` - 订单列表
  - `/admin/refunds` - 退款管理（待开发）
  - `/admin/cards` - 卡密列表（待开发）
  - `/admin/card-generate` - 生成卡密（待开发）
  - `/admin/settings` - 系统设置
  - `/admin/logs` - 操作日志（待开发）

## 登录信息

**演示账号：**
- 用户名：`admin`
- 密码：`admin`

登录成功后会将token保存到localStorage，实现持久化登录。

## 配置说明

### API 代理

在 `vite.config.ts` 中配置了 API 代理，将 `/api` 请求代理到后端服务器：

```typescript
server: {
  port: 3001,  // 后台管理端口 3001
  proxy: {
    '/api': {
      target: 'http://localhost:8080',
      changeOrigin: true,
    },
  },
}
```

### Tailwind CSS

在 `tailwind.config.js` 中禁用了 `preflight`，以避免与 Naive UI 的样式冲突：

```javascript
corePlugins: {
  preflight: false,
}
```

## 路由守卫

系统实现了路由守卫功能：
- 访问后台页面时会检查token
- 未登录自动跳转到登录页
- 已登录访问登录页会跳转到控制台

## 开发建议

1. 使用 Composition API 编写组件
2. 遵循 TypeScript 类型检查
3. 使用 Tailwind 实用类进行样式设计
4. 使用 Naive UI 组件构建界面
5. 使用 Pinia 进行状态管理
6. 使用封装的 http 工具进行 API 请求

## 浏览器支持

- Chrome >= 87
- Firefox >= 78
- Safari >= 14
- Edge >= 88

## 待开发功能

- [ ] 角色管理页面
- [ ] 退款管理页面
- [ ] 卡密列表页面
- [ ] 生成卡密页面
- [ ] 操作日志页面
- [ ] 个人设置页面
- [ ] 修改密码页面
- [ ] 图表集成（建议使用 ECharts）
- [ ] 权限控制系统
- [ ] 数据导出功能
- [ ] 批量操作功能
- [ ] 实时数据更新
- [ ] WebSocket 支持

## 与前台项目的关系

- 前台项目：`web/` - 端口 3000
- 后台项目：`admin/` - 端口 3001

两个项目完全独立，可以单独部署和运行。

