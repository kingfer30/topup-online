...
func Debug(ctx context.Context, msg string) {
	if constants.DebugEnabled {
		logHelper(ctx, constants.LoggerDEBUG, msg)
	}
}

func Info(ctx context.Context, msg string) {
	logHelper(ctx, constants.LoggerINFO, msg)
}

func Warn(ctx context.Context, msg string) {
	logHelper(ctx, constants.LoggerWarn, msg)
}

func Error(ctx context.Context, msg string) {
	logHelper(ctx, constants.LoggerError, msg)
}
...
func logHelper(ctx context.Context, level string, msg string) {
	...
	if level == constants.LoggerINFO {
		writer = gin.DefaultWriter
	}
	id := ctx.Value(constants.RequestIdKey)
	if id == nil {
